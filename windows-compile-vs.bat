...
call "C:\Program Files (x86)\Microsoft Visual Studio\2017\Enterprise\VC\Auxiliary\Build\vcvarsall.bat" %ARCH% >>"%log_file%" 2>&1 || call :pm-fatal-error "Error initializing Visual Studio environment"
...
del /q pocketmine-php-sdk\php-src\%ARCH%\Release_TS\php-%PHP_VER%\icu*.dll
...
del /q pocketmine-php-sdk\php-src\%ARCH%\Release_TS\php-%PHP_VER%\glib-*.dll
del /q pocketmine-php-sdk\php-src\%ARCH%\Release_TS\php-%PHP_VER%\gmodule-*.dll
rmdir /s /q pocketmine-php-sdk\php-src\%ARCH%\Release_TS\php-%PHP_VER%\lib\enchant\

call :pm-echo "Copying artifacts..."
cd "%outpath%"
mkdir bin
move pocketmine-php-sdk\php-src\%ARCH%\Release_TS\php-%PHP_VER% bin\php
cd bin\php
